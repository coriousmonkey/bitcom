...
class Checkout extends CI_Controller {
	...
	function index()
	{
		...
		if($session!="")
		{
			...
			$data['judul'] = "Selesai Belanja - BitCom Jember, Toko Komputer Termurah dan Terlengkap di Jember - BitCom Jember";
	...
	function update_keranjang()
	{
	    $formdata="";
        error_reporting(0);
        $formdata=$this->input->post('kurir');
        ...
        redirect(base_url()."checkout/?".$formdata);
	//	echo "<meta http-equiv='refresh' content='0; url=".base_url()."checkout/'>";
	}
	...
	function kirim_invoice()
	{
		...
		if($session==""){
			...
			$data["email"]=$pecah[3];
			$data['menu'] = $this->sandal_model->menu_kategori('0','0');
			$data['judul'] = "Kirim Pesanan - BitCom Online, Toko Komputer Termurah dan Terlengkap di Jember - BitCom Jember";
			$data['slide_atas'] = $this->sandal_model->tampil_slide_produk(10);
			$data['slide_laris'] = $this->sandal_model->tampil_slide_produk_terlaris_kiri(5);
			$data['slide_baru'] = $this->sandal_model->tampil_produk(5);
			$data['slide_rekomendasi'] = $this->sandal_model->tampil_slide_produk(6);
			...
			$data['pesan'] = "";
				if($datapesan['nama_penerima']=="" || $datapesan['email_penerima']=="" || $datapesan['alamat_penerima']=="" || $datapesan['kota_penerima']=="" || $datapesan['kodepos_penerima']=="" || $datapesan['telpon_penerima']=="" )
				{
				...
				{
					$isi_psn = 'Terima kasih telah berbelanja di BitCom Online. Berikut detail produk yang anda beli :<br><br>
...
';
					...
					$isi_psn .='Salam, Harmonis Perintis Furniture';
					
					$this->email->set_mailtype('html');
					$this->email->from("admin@perintisfurniture.com", "Admin BitCom Online");
					$this->email->to($data["email"]);
					$this->email->subject('Detail Pesanan/Belanja - Member BitCom Online');
					...
					$this->email->to("admin@perintisfurniture.com");
					$this->email->subject('Detail Pesanan/Belanja - Member Perintis Furniture Online');
					$this->email->message($isi_psn);	
